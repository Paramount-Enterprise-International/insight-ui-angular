...
export type IDropdownOptionContext<T> = {
  $implicit: T;
  row: T;
};
...
export class IDropdownOptionDefDirective<T = any> {
  template = inject<TemplateRef<IDropdownOptionContext<T>>>(TemplateRef);
...
export type IDropdownChange<T = any> = {
  value: T | null;
  label: string;
};

/** Position of popup options relative to the input */
export type IDropdownPanelPosition =
  | 'top'
  | 'bottom'
  | 'left'
  | 'right'
  ...
  providers: [
    {
      ...
      useExisting: forwardRef(() => IDropdown),
      multi: true,
    },
  ],
})
export class IDropdown<T = any>
  implements ControlValueAccessor, OnInit, AfterContentInit, AfterViewChecked, OnDestroy
{
  ...
  @Input() panelPosition: IDropdownPanelPosition = 'bottom left';
  ...
  // ---------- Outputs ----------
  @Output() readonly onChanged = new EventEmitter<IDropdownChange<T>>();
  @Output() readonly onOptionSelected = new EventEmitter<IDropdownChange<T>>();

  // ---------- Template refs ----------
  @ContentChild(IDropdownOptionDefDirective)
  optionDef?: IDropdownOptionDefDirective<T>;
  ...
  selectRow(row: T): void {
    ...
    const payload: IDropdownChange<T> = {
      value: row,
...
@Component({
  selector: 'i-fc-dropdown',
  standalone: true,
  imports: [IDropdown],
  template: `
    @if (label) {
      <label class="i-fc-dropdown__label" (click)="focusInnerSelect()">
        {{ label }} :
        @if (required) {
          <span class="i-fc-dropdown__required">*</span>
        }
      </label>
    }
    ...
    @if (controlInvalid && resolvedErrorText) {
      <div class="i-fc-dropdown__error">
        {{ resolvedErrorText }}
      </div>
    }
  `,
...
export class IFCDropdown<T = any> implements ControlValueAccessor, OnDestroy, AfterViewInit {
  @ViewChild(IDropdown) innerSelect!: IDropdown<T>;
  ...
  handleSelectChange(change: IDropdownChange<T>): void {
