...
import { firstValueFrom, Subject, BehaviorSubject, map, filter, startWith, shareReplay, combineLatest, Observable, tap } from 'rxjs';
...
class IHContent {
    ...
    routeBreadcrumb$ = this.router.events.pipe(filter((e) => e instanceof NavigationEnd), startWith(null), // emit once on init
    map(() => this.buildBreadcrumb(this.activatedRoute.root)), shareReplay(1));
    /** override breadcrumbs (read from signals) */
    overrideBreadcrumb$ = this.router.events.pipe(filter((e) => e instanceof NavigationEnd), startWith(null), map(() => this.toBreadcrumbItems(this.shell.breadcrumbsOverride())), shareReplay(1));
    /** final breadcrumbs: override wins */
    breadcrumb$ = combineLatest([
        this.routeBreadcrumb$,
        this.overrideBreadcrumb$,
    ]).pipe(map(([routeCrumbs, override]) => override ?? routeCrumbs), shareReplay(1));
    /** title: override wins, else last breadcrumb label */
    pageTitle$ = this.breadcrumb$.pipe(map((breadcrumbs) => {
        const titleOverride = this.shell.titleOverride();
        if (titleOverride)
            return titleOverride;
        return breadcrumbs.length > 0 ? breadcrumbs[breadcrumbs.length - 1].label : null;
    }), shareReplay(1));
    toBreadcrumbItems(items) {
        if (!items)
            return null;
